...
class db_mysql
{
    ...
    public function real_connect($host, $user, $password, $name, $charset = '', $engine = '')
    {
        ...
        if (!$link) {
            $this->error(mysql_errno(), '连接数据库服务器失败:' . mysql_error());
            return FALSE;
        }
        if (!mysql_select_db($name, $link)) {
            $this->error(mysql_errno(), '选择数据库失败:' . mysql_error());
            return FALSE;
        }
...
class db_pdo_sqlite
{
    ...
    public function real_connect($host, $user, $password, $name, $charset = '', $engine = '')
    {
        ...
        try {
            $attr = array(PDO::ATTR_TIMEOUT => 5, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION);
...
class cache_mysql
{
    ...
    public function set($k, $v, $life = 0)
    {
        ...
        $arr = array('k' => $k, 'v' => xn_json_encode($v), 'expiry' => $expiry,);
        ...
        foreach ($arrlist as $key => $val) {
            ...
            foreach ($cond as $k => $v) {
                ...
                if (!is_array($v)) {
                    ...
                    foreach ($v as $k3 => $v3) {
                        if (($k3 == '>' && $val[$k] <= $v3) || ($k3 == '<' && $val[$k] >= $v3) || ($k3 == '>=' && $val[$k] < $v3) || ($k3 == '<=' && $val[$k] > $v3) || ($k3 == '==' && $val[$k] != $v3) || ($k3 == 'LIKE' && stripos($val[$k], $v3) === FALSE)) {
                            $ok = FALSE;
                            break 2;
                        }
                    }
...
function humandate($timestamp, $lan = array())
{
    ...
    empty($lan) AND $lan = array('month_ago' => '月前', 'day_ago' => '天前', 'hour_ago' => '小时前', 'minute_ago' => '分钟前', 'second_ago' => '秒前',);
    if ($seconds > 31536000) {
...
{
    $browser = array('device' => 'pc', 'name' => 'chrome', 'version' => 30,);
...
function http_get($url, $cookie = '', $timeout = 30, $times = 3)
{
    ...
    $arr = array('http' => array('method' => 'GET', 'timeout' => $timeout));
